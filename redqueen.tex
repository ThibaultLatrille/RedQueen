...
&=& \int_0^\infty x(t) f\left(  \frac{\theta_0 + \theta(t)}{2} \right) dt 
\\
...
\begin{eqnarray*}
\bar{f} &=& \int_0^\tau \int_0^\tau f \left(  \frac{ e^{-\rho \xi_1} +  e^{-\rho \xi_2}}{2} \right) d \xi_1 d \xi_2
\\
...
We thus recover all scaling relations that were pointed out in the previous section, under arbitrary fitness models.
...
\end{eqnarray*}
The total (relative) recombination activity induced by a PRDM9 allele with intrinsic age $\xi$ is then obtained by averaging $\theta_r(t)$ over $r$ (under the gamma), which happens to be analytical:
\begin{eqnarray*}
\theta(\xi) &=& \int c \theta_c \phi(c) dc = \left( \frac{a}{a + \rho \xi} \right)^{a+1}
\end{eqnarray*}
The mean recombination activity over the population is then obtained by averaging over $\xi$:
\begin{eqnarray}
\label{powertheta}
\theta = \frac{1}{\tau} \int_0^\tau R(\xi) d \xi &=& \frac{1}{\rho \tau} \left[ 1 - \left( \frac{a}{a + \rho \tau} \right)^a \right]
\end{eqnarray}
