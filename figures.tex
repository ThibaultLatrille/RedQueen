...
\begin{figure*}[hbt!]
	...
	\node at (axis cs:0.3,0.5) [anchor=south west, color1] {$R=R$};
	\node at (axis cs:0.62,0.84) [anchor=south west, color2] {$\tfrac{\epsilon}{\alpha}=0.01$};
	\node at (axis cs:0.4,0.73) [anchor=south west, color3] {$\tfrac{\epsilon}{\alpha}=0.1$};
	
	\end{axis}
	\end{tikzpicture}
	\caption{Self-consistent solution as a fixed point, in the form $R = g(R) = \dfrac{\alpha}{\epsilon} \dfrac{1-R}{2 R} \left( 1- e^{- \tfrac{\epsilon}{\alpha}\tfrac{2 R}{(1-R)}} \right)$, for two values of $\dfrac{\epsilon}{\alpha} = \dfrac{vg}{\alpha u}$.\label{fig:equation}}
\end{figure*}
\end{document}
